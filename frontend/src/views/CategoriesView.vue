...
<template>
  <section class="view-stack">
    ...
    <form class="panel merchant-form category-create-form" @submit.prevent="create">
      ...
      <label class="field"><span>Priority</span><select v-model="defaultPriority"><option value="protected">Protected</option><option value="essential">Essential</option><option value="adjustable">Adjustable</option><option value="optional">Optional</option><option value="irregular_essential">Irregular essential</option><option value="transfer">Transfer</option></select></label>
    ...
    <div class="category-tree">
      <article v-for="root in roots" :key="root.id" class="panel category-group">
        <div class="category-edit"><input v-model="root.name" /><select v-model="root.default_priority" class="table-input"><option value="protected">Protected</option><option value="essential">Essential</option><option value="adjustable">Adjustable</option><option value="optional">Optional</option><option value="irregular_essential">Irregular essential</option><option value="transfer">Transfer</option></select><button class="text-button" @click="save(root)">Save</button><button class="text-button danger" @click="remove(root)">Delete</button></div>
        <div class="child-list">
          <div v-for="child in children(root)" :key="child.id" class="category-edit child-category"><span>↳</span><input v-model="child.name" /><select v-model="child.default_priority" class="table-input"><option value="protected">Protected</option><option value="essential">Essential</option><option value="adjustable">Adjustable</option><option value="optional">Optional</option><option value="irregular_essential">Irregular essential</option><option value="transfer">Transfer</option></select><button class="text-button" @click="save(child)">Save</button><button class="text-button danger" @click="remove(child)">Delete</button></div>
        </div>
      </article>
    </div>
  </section>
