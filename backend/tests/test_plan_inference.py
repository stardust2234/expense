...
def test_infers_income_commitments_and_variable_essential_allowance() -> None:
    ...
    assert preview.income.expected_amount == 90000
    assert preview.income.payment_date == date(2026, 8, 29)
    assert preview.income.evidence_transaction_ids == (1, 2, 3)
...
def test_requires_repeated_monthly_income() -> None:
    ...
    with pytest.raises(InsufficientPlanEvidenceError, match="At least two monthly"):
        infer_plan(
            evidence,
            target_month=date(2026, 8, 1),
            currency="GBP",
